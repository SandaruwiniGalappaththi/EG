...
/*public String readByZone(String zoneLetter) {
	 	...
	 	if (con == null)
 {
	...
	output += "</table>";
	if(existzone == false) {
		return output = "REQUESTED ZONE IS NOT AN AVAILABLE ZONE!!!" + existzone +existschedule; 
	 }	
		if(existschedule == false) {
		return output = "NO SCHEDULE ADDED YET FOR YOUR AREA!!!" + existzone +existschedule; 
	 }	
	return output + existzone +existschedule;
}catch(Exception e) {
	return "";
    }
  }
